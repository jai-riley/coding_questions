...
class Solution(object):
    ...
    def intToRoman(self, num):
        ...
        if num == 0:
            return ""
        zeros = len(str(num)) - 1
        if zeros >= 3 or int(str(num)[0]) < 4:
            return self.conversion["1" + ("0" * zeros)] + self.intToRoman(num - int("1" + ("0" * zeros)))
        else:
            if str(num)[0] in ["1", "4", "5", "9"]:
                return self.conversion[str(num)[0] + ("0" * zeros)] + self.intToRoman(
                    num - int(str(num)[0] + ("0" * zeros)))
            elif int(str(num)[0]) > 5:  # conversely could be just else but this speeds up runtime
                return self.conversion["5" + ("0" * zeros)] + self.intToRoman(num - int("5" + ("0" * zeros)))
